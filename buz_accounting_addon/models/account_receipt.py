...
class BuzAccountReceiptSettings(ResConfigSettings):
    ...
    buz_receipt_autopost = fields.Boolean(
        ...
        default=True,
        config_parameter='buz_accounting_addon.auto_post_receipts',
    ...
    buz_enforce_single_currency_per_receipt = fields.Boolean(
        ...
        default=True,
        config_parameter='buz_accounting_addon.enforce_single_currency_per_receipt',
...
class AccountReceipt(models.Model):
    ...
    def _check_receipt_lines_currency(self):
        ...
        single_currency_enabled = self.env['ir.config_parameter'].sudo().get_param(
            'buz_accounting_addon.enforce_single_currency_per_receipt', default=True
        )
    ...
    def action_print_receipt(self):
        self.ensure_one()
        return self.env.ref("buz_accounting_addon.action_report_buz_accounting_addon").report_action(self)
    ...
    def _compute_used_moves(self):
        ...
        """
        self.env.cr.execute("""
            SELECT DISTINCT rl.move_id
            FROM account_receipt_line rl
            JOIN account_receipt r ON rl.receipt_id = r.id
            WHERE rl.move_id IS NOT NULL
              AND r.company_id IN %s
    ...
    def action_register_batch_payment(self):
        ...
        if invoices:
            ...
            allow_outstanding_fallback = self.env['ir.config_parameter'].sudo().get_param(
                'buz_accounting_addon.allow_outstanding_fallback', default=True
            )
            ...
            if allow_outstanding_fallback and self.amount_total > 0:
                ...
                default_journal_id = self.env['ir.config_parameter'].sudo().get_param(
                    'buz_accounting_addon.default_bank_journal_id'
                )
                journal = None
    ...
    def _compute_used_in_voucher(self):
        ...
        existing = self.env['account.receipt.voucher.line'].search([
            ...
            ('voucher_id.company_id', 'in', self.env.companies.ids),
        ])
