...
class MarketplaceSettlementWizard(models.TransientModel):
    ...
    trade_channel_id = fields.Many2one('marketplace.trade.channel', string='Trade Channel', required=True)
    ...
    def default_get(self, fields_list):
        ...
        if profile_id:
            ...
            if profile.trade_channel_id:
                res['trade_channel_id'] = profile.trade_channel_id.id
    ...
    @api.onchange('trade_channel_id', 'date_from', 'date_to')
    def _onchange_trade_channel_id(self):
        ...
        if self.trade_channel_id and not self.profile_id:
            Profile = self.env['marketplace.settlement.profile']
            matching_profile = Profile.search([
                ('trade_channel_id', '=', self.trade_channel_id.id),
                ('active', '=', True)
            ], limit=1)
        ...
        if self.trade_channel_id and not self.profile_id:
            ...
            prof = Profile.search([('trade_channel_id', '=', self.trade_channel_id.id)], limit=1)
            if prof:
        ...
        if self.trade_channel_id and self.auto_filter:
            ...
            if not self.name or self.name.startswith('SETTLE-'):
                ...
                self.name = f'SETTLE-{self.trade_channel_id.code.upper() if self.trade_channel_id else ""}-{date_suffix}'

        # Return domain for manual selection in the view
        if self.trade_channel_id:
            domain = [
                ('state', '=', 'posted'),
                ('move_type', 'in', ['out_invoice', 'out_refund']),
                ('trade_channel_id', '=', self.trade_channel_id.id),
    ...
    @api.onchange('auto_filter', 'trade_channel_id')
    def _onchange_auto_filter(self):
        """Toggle auto-filtering behavior"""
        if self.auto_filter and self.trade_channel_id:
            self._onchange_trade_channel_id()
    ...
    def _onchange_profile(self):
        ...
        if prof.trade_channel_id:
            self.trade_channel_id = prof.trade_channel_id.id
    ...
    def action_create(self):
        ...
        settlement = self.env['marketplace.settlement'].create({
            ...
            'date': self.date,
            'trade_channel_id': self.trade_channel_id.id if self.trade_channel_id else False,
