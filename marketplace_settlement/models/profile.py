...
class MarketplaceSettlementProfile(models.Model):
    ...
    @api.model
    def get_profile_by_channel(self, trade_channel_code):
        """Get active profile for a trade channel code"""
        return self.search([
            ('trade_channel_id.code', '=', trade_channel_code),
            ('active', '=', True)
        ], limit=1)
    ...
    def _onchange_trade_channel_id(self):
        ...
        if self.trade_channel_id:
            # Set default name
            self.name = f"{self.trade_channel_id.name} Profile"
            
            # Set default patterns based on channel
            if self.trade_channel_id.code == 'shopee':
                self.invoice_pattern = 'TR'
            elif self.trade_channel_id.code == 'spx':
                self.receipt_pattern = 'RC'
            elif self.trade_channel_id.code == 'lazada':
                self.invoice_pattern = 'LZ'
            elif self.trade_channel_id.code == 'tiktok':
    ...
    def action_create_settlement_with_profile(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_trade_channel_id': self.trade_channel_id.id if self.trade_channel_id else False,
    ...
    def action_create_vendor_bill_with_profile(self):
        ...
        vendor_bill_values = {
            ...
            'ref': f'{self.trade_channel_id.name.upper() if self.trade_channel_id else ""}-FEES-{fields.Date.context_today(self)}',
        }
