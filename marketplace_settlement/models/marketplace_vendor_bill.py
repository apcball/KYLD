...
class MarketplaceVendorBill(models.Model):
    ...
    trade_channel_id = fields.Many2one('marketplace.trade.channel', string='Trade Channel', help='Trade channel for this vendor bill')
    ...
    def _onchange_document_type(self):
        ...
        if self.document_type == 'shopee_tr':
            self.trade_channel_id = self.env['marketplace.trade.channel'].search([('code', '=', 'shopee')], limit=1).id
            ...
            self.trade_channel_id = self.env['marketplace.trade.channel'].search([('code', '=', 'spx')], limit=1).id
            # Suggest profile but don't force it
    ...
    def _onchange_use_manual_config(self):
        ...
        if self.use_manual_config:
            ...
            if self.trade_channel_id:
                profile = self.env['marketplace.settlement.profile'].get_profile_by_channel(self.trade_channel_id.code)
                if profile:
                    self.profile_id = profile.id
                    self._apply_profile_defaults()

    @api.onchange('trade_channel_id')
    def _onchange_trade_channel_id(self):
        """Update profile suggestion when trade channel changes"""
        if self.trade_channel_id and not self.use_manual_config:
            profile = self.env['marketplace.settlement.profile'].get_profile_by_channel(self.trade_channel_id.code)
            if profile:
                self.profile_id = profile.id
                self._apply_profile_defaults()
...
class MarketplaceVendorBillLine(models.Model):
    ...
    def _onchange_document_type(self):
        ...
        if self.bill_id.use_manual_config:
            ...
            if self.bill_id.trade_channel_id and self.bill_id.trade_channel_id.code == 'shopee':
                self.vat_rate = 7.0
                self.wht_rate = 3.0
            elif self.bill_id.trade_channel_id and self.bill_id.trade_channel_id.code == 'spx':
                self.vat_rate = 0.0
                self.wht_rate = 1.0
            else:
                self.vat_rate = 0.0
