...
class BudgetMove(models.Model):
            ...
            if rec.source_model and rec.source_id:
                try:
                    ...
                    if source_doc.exists():
                        ...
                        company = getattr(source_doc, 'company_id', None) or False
                except Exception as e:
                    # Fix: Log the exception instead of silently passing
                    _logger.warning(
                        "Budget move: failed to compute company for model=%s, id=%s: %s",
                        rec.source_model, rec.source_id, e
                    )

            # Fallback to department's company if source is missing
            if not company and rec.department_id:
                company = getattr(rec.department_id, 'company_id', None) or False
    ...
    def extract_analytic_distribution(self, line):
        ...
        res = []
        # Fix: Use getattr with default to avoid AttributeError
        acc_id = getattr(line, 'analytic_account_id', None) or False
        dept_id = False  # Will be resolved below in priority order

        # -----------------------------------------------------------
        # Priority 1: MR source department (authoritative for budget)
        # -----------------------------------------------------------
        mr_line_linked = getattr(line, 'material_requisition_line_id', None)
        if mr_line_linked and mr_line_linked.requisition_id:
            mr_dept = getattr(mr_line_linked.requisition_id, 'department_id', None)
            if mr_dept:
                dept_id = mr_dept
            if not acc_id:
                acc_id = getattr(mr_line_linked, 'analytic_account_id', None) or getattr(mr_line_linked.requisition_id, 'analytic_account_id', None) or False
        ...
        if not dept_id:
            ...
            if order:
                pr_name = getattr(order, 'requisition_order', None) or getattr(order, 'pr_number', None) or False
                if pr_name:
                    pr = self.env['employee.purchase.requisition'].sudo().search(
                        [('name', '=', pr_name)], limit=1)
                    if pr:
                        pr_dept = getattr(pr, 'dept_id', None) or getattr(pr, 'department_id', None) or False
                        if pr_dept:
                            dept_id = pr_dept
        ...
        # -----------------------------------------------------------
        if not dept_id:
            dept_id = getattr(line, 'department_id', None) or False
        ...
        order = (
            getattr(line, 'order_id', None)
            or getattr(line, 'requisition_id', None)
            or getattr(line, 'material_requisition_id', None)
            or getattr(line, 'requisition_product_id', None)
        )

        if not order:
            # Bill line → linked PO via purchase_line_id
            po_line = getattr(line, 'purchase_line_id', None)
            if po_line:
                order = getattr(po_line, 'order_id', None)

        if not order:
            # Last resort: the parent document itself (e.g., the bill header)
            order = getattr(line, 'move_id', None)

        if order:
            if not acc_id:
                acc_id = getattr(order, 'analytic_account_id', None) or getattr(order, 'expense_code_id', None) or False
            if not dept_id:
